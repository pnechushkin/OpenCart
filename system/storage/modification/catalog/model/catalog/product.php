...
class ModelCatalogProduct extends Model {
    ...
    if($timer_exist){
      $timer_query .= "
        (SELECT date_start FROM " . DB_PREFIX . "product_special ps WHERE ps.product_id = p.product_id AND ps.customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "' AND (ps.date_start = '0000-00-00' OR ps.date_start < NOW()) ORDER BY ps.date_start ASC LIMIT 1) AS date_start, 
        (SELECT date_end FROM " . DB_PREFIX . "product_special ps2 WHERE ps2.product_id = p.product_id AND ps2.customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "' AND (ps2.date_end = '0000-00-00' OR ps2.date_end > NOW()) ORDER BY ps2.date_end ASC LIMIT 1) AS date_end, 
        (SELECT timer FROM " . DB_PREFIX . "product_special ps3 WHERE ps3.product_id = p.product_id AND ps3.customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "' AND (ps3.date_end = '0000-00-00' OR ps3.date_end > NOW()) ORDER BY ps3.date_end ASC LIMIT 1) AS timer, ";
    }
