...
class ControllerBlogHome extends Controller {
	...
	public function index() { 
		...
		if( $filter_tag ){
				...
				$data['heading_title'] = $blog_title[$this->language->get('code')];
			}
		
		$blog_page_title = $this->config->get('blogsetting_home_page_title');
		if ($blog_page_title[$this->language->get('code')]) {
		$this->document->setTitle($blog_page_title[$this->language->get('code')]);
		} else {
		$this->document->setTitle($blog_title[$this->language->get('code')]);
		}
		
		$blog_page_meta_description = $this->config->get('blogsetting_home_meta_description');
		if ($blog_page_meta_description[$this->language->get('code')]) {
		$this->document->setDescription($blog_page_meta_description[$this->language->get('code')]);
		}
		
		$blog_page_meta_keyword = $this->config->get('blogsetting_home_meta_keyword');
		if ($blog_page_meta_keyword[$this->language->get('code')]) {
		$this->document->setKeywords($blog_page_meta_keyword[$this->language->get('code')]);
		}
		
		
		$blog_description = $this->config->get('blogsetting_home_description');
		
		if ($blog_description[$this->language->get('code')] == '&lt;p&gt;&lt;br&gt;&lt;/p&gt;') {
			$data['description'] = false;
			} else {
			$data['description'] = html_entity_decode(($blog_description[$this->language->get('code')]), ENT_QUOTES, 'UTF-8');
			}
