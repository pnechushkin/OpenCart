...
class ControllerModuleShopmeRecently extends Controller {
	public function index($setting) {
        ...
        if (isset($this->request->cookie['shopme_recently'])) {
            $products = explode(',', $this->request->cookie['shopme_recently']);
        } else if (isset($this->session->data['shopme_recently'])) {
            $products = $this->session->data['shopme_recently'];
        }
        ...
        if (isset($this->request->get['route']) && $this->request->get['route'] == 'product/product') {
            ...
            setcookie('shopme_recently', implode(',',$products), time() + 60 * 60 * 24 * 30, '/', $this->request->server['HTTP_HOST']);
        }
		...
		if ($data['products']) {
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/module/shopme_recently.tpl')) {
				return $this->load->view($this->config->get('config_template') . '/template/module/shopme_recently.tpl', $data);
			} else {
				return $this->load->view('default/template/module/shopme_recently.tpl', $data);
			}
		}
	}
}