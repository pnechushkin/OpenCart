...
<div style="clear: both; padding:9px 0">
  <?php if (!empty($field_register['display'])) { ?>
	...
	<?php } else { ?>
	  <input type="checkbox" name="create_account" value="1" id="create"<?php echo !empty($field_register['default']) ? ' checked="checked"' : ''; ?> /> <label for="create" style="margin-bottom:3px;"><?php echo $text_create_account; ?></label><br />
	  <div id="create_account"></div>
	<?php } ?>
  <?php } else { ?>
