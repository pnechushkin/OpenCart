<?php
class ModelModuleShopmeTagcloud extends Model {
	...
	if ($tags) {
		foreach ($tags as $record) { 
			...
			$cloud[] = '<a class="btn btn-default btn-tag" href="' . $this->url->link(str_replace('&', '&amp;', $tag_href)) . '" title="' . $record['tag'] . '">' . $record['tag'] . '</a>';
			}
			$tagcloud = '';
