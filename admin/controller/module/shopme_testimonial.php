	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			if (!isset($this->request->get['module_id'])) {
				$this->model_extension_module->addModule('shopme_testimonial', $this->request->post);
			} else {
		...
		$data['to_list_link'] = $this->url->link('catalog/testimonial', 'token=' . $this->session->data['token'], 'SSL');
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('module/shopme_testimonial', 'token=' . $this->session->data['token'], 'SSL')
		);

		if (!isset($this->request->get['module_id'])) {
			$data['action'] = $this->url->link('module/shopme_testimonial', 'token=' . $this->session->data['token'], 'SSL');
		} else {
			$data['action'] = $this->url->link('module/shopme_testimonial', 'token=' . $this->session->data['token'] . '&module_id=' . $this->request->get['module_id'], 'SSL');
		}
		...
		if (isset($this->request->post['testimonial_contrast'])) {
			$data['testimonial_contrast'] = $this->request->post['testimonial_contrast'];
		} elseif (!empty($module_info['testimonial_contrast'])) {
			$data['testimonial_contrast'] = $module_info['testimonial_contrast'];
		} else {
		...
		$this->response->setOutput($this->load->view('module/shopme_testimonial.tpl', $data));
	}

	protected function validate() {
		if (!$this->user->hasPermission('modify', 'module/shopme_testimonial')) {
